-- The mod's readable books (content pak, tools/pakkit). Two of them share this one feature:
--
--   * The TEMPEST CODEX   -- the dark arts: unlock "The Dark Arts", craft at the bench, place,
--                            read. Carries the research-card save migration.
--   * The TEMPEST HANDBOOK -- a plain-spoken summary of everything this mod adds. A STARTING
--                            recipe in the quick-craft (F) menu, so a new player finds it in
--                            their first minute. No research, hence no migration.
--
-- The pak clones the survival guide's whole data-driven chain PER BOOK: W_<book> (the reader UI)
-- pulls its sections from the cooked DB_<book> table, and BP_<book>_Placeable is the placed,
-- interactable book. Nothing is shared between the two chains -- the category chip reads its own
-- enum's DisplayNameMap, so a shared widget would render the other book's section titles.
--
-- The cooked placeable's native "open survival guide" virtual call was retargeted to the
-- controller's no-op ForceCloseInteractableUIs, so OPENING the reader is this feature's job: hook
-- each clone's interact event, create its widget once (the game's own controller does the same
-- for the survival guide in StartupUI: Create -> AddToViewport, then Open per read), and Open it.
--
-- STATE SPLIT. Widget, hooks and diagnostics are PER BOOK. Input focus and the UI_SurvivalGuide
-- repoint are SHARED, because they are properties of the CONTROLLER, not of a book: there is one
-- input mode and one guide slot, and only one book can be on screen at a time.
--
-- MP: the interact event runs where the interaction is processed; the UI must only open for the
-- LOCAL player, so the hook compares the interacting controller against ours. Everything is
-- pcall-guarded and hook-driven -- no UObject is ever polled on a timer.
local F = {}
local ctx

local books = {}          -- every gated book, in registration order
-- Controller-scoped, therefore shared by every book.
local shared = {
  guideOriginal = nil,    -- the REAL survival-guide widget while we are repointed
  repointOwner  = nil,    -- the book currently holding pc.UI_SurvivalGuide (or nil)
  activeBook    = nil,    -- the book that owns the screen (or nil)
  uiFocused     = false,
  gameModeHooked = false,
}

local function onGameThread(fn)
  if ExecuteInGameThread then
    if pcall(ExecuteInGameThread, fn) then return end
  end
  pcall(fn)
end

-- ---------------------------------------------------------------- the guide-slot arbiter
-- Ride the game's own interactable-UI registry (offline RE 2026-07-22, the "can still walk
-- around while reading" bug): the pawn gates movement, hotbar scroll and tool input on the
-- controller's `Is An Interactable UIOpen` -- a HARDCODED OR over ~28 named widget properties
-- (UI_SurvivalGuide among them, checked via IsVisible()); ESC closes through
-- CloseOpenInteractableUIs (Close() on each) and cinematics hide via HideAllUI (Hide() on
-- each). There is no dynamic registry to enroll in, but every controller reference to
-- UI_SurvivalGuide is a BY-NAME virtual our clones implement identically (IsVisible / Close /
-- Hide -- verified against the full reference map: StartupUI, the guide-open event,
-- Is An Interactable UIOpen, CloseOpenInteractableUIs, HideAllUI). So while a book is open we
-- REPOINT pc.UI_SurvivalGuide at its widget and restore it on close: one property write buys the
-- complete guidebook behavior -- char lock, hotbar lock, ESC close.

-- With TWO books this needs an arbiter. The failure it prevents: book A's repoint survives a
-- close path that dodged every hook, so the slot still holds widget_A. Book B then reads the
-- slot, finds a valid, hidden widget, and captures OUR OWN WIDGET as "the real guide" -- and
-- writes it back on close. The vanilla guide is orphaned for the rest of the session.
local function isOurWidget(o)
  if not o then return false end
  for _, b in ipairs(books) do
    if b.widget and ctx.uehelp.sameObject(b.widget, o) then return true end
  end
  return false
end

local function restoreGuide()
  local b = shared.repointOwner
  if not b then return end
  shared.repointOwner = nil
  local u, m = ctx.uehelp, b.map()
  local pc = u.playerController()
  if pc and u.isValid(shared.guideOriginal) then
    u.set(pc, m.guideProp or "UI_SurvivalGuide", shared.guideOriginal)
  end
  shared.guideOriginal = nil
end

local function repointGuide(pc, b)
  local u, m = ctx.uehelp, b.map()
  if shared.repointOwner == b then return true end
  if shared.repointOwner then restoreGuide() end   -- hand the slot back before taking it
  local okG, orig = u.get(pc, m.guideProp or "UI_SurvivalGuide")
  if not okG or not u.isValid(orig) then return false end   -- no live guide widget: nothing to ride
  if isOurWidget(orig) then
    -- a previous repoint leaked. NEVER adopt one of our own widgets as "the real guide" -- that
    -- write would orphan the vanilla guide permanently. Self-heal from the stashed original.
    if not u.isValid(shared.guideOriginal) then return false end
    orig = shared.guideOriginal
  end
  local vis = false
  pcall(function() vis = orig:IsVisible() end)
  if vis then return false end                     -- the REAL guide is open: don't hijack it
  shared.guideOriginal, shared.repointOwner = orig, b
  u.set(pc, m.guideProp or "UI_SurvivalGuide", b.widget)
  return true
end

-- Input focus. The widget never touches input mode itself (offline RE: for the survival guide
-- the CONTROLLER's ubergraph pairs the widget's Open with SetInputModeUI and the close path with
-- SetInputModeGame; the widget just shows/hides). The controller doesn't know our widgets, so we
-- make its exact calls ourselves -- the guide's own call site reads
-- SetInputModeUI(UI_SurvivalGuide, DontQuitBuildingMode=false, LockMovement=IsGamepad,
-- ShowCursorOnGamepad=false, DisableTabNavigation=true), and every UI closes through
-- SetInputModeGame(false, false, false).
--
-- Two hard-won details (live 2026-07-22, "can still move around and swap items while reading"):
--   * SetInputModeUI's ENGINE half (bShowMouseCursor + WidgetBlueprintLibrary
--     SetInputMode_GameAndUIEx, the part that actually stops WASD/hotbar keys reaching the game)
--     lives in gated ubergraph cursor state (UpdateGamepadCursor) that is not reliable from an
--     out-of-band call site -- so after the game's own call we make the engine-level pair
--     ourselves. Idempotent when the gated block did run.
--   * See book.open for WHEN this may be called: never in the interact frame itself.
local function setUiFocus(on, b)
  local u, m = ctx.uehelp, b.map()
  local pc = u.playerController()
  if not pc then return end
  local wbl = StaticFindObject and StaticFindObject(m.wblPath)
  if on then
    -- activeBook tracks WHO IS ON SCREEN, so it is set even when the focus call fails: the
    -- book is still open, and the next book to open still has to shut this one first.
    shared.activeBook = b
    local ok = u.call(pc, m.inputUiFn or "SetInputModeUI", b.widget, false, false, false, true)
    if ok then shared.uiFocused = true
    else ctx.log.info(b.label .. ": input-mode call failed -- the book opens without focus") end
    -- engine half: cursor on + route input through the UI (the controller's own KB/M block does
    -- exactly SetInputMode_GameAndUIEx(self, widget, 0, false, false) + bShowMouseCursor = true)
    if wbl then
      pcall(function() wbl:SetInputMode_GameAndUIEx(pc, b.widget, 0, false, false) end)
    end
    u.set(pc, "bShowMouseCursor", true)
  elseif shared.activeBook == b then
    -- clear the screen owner even if the focus grab never succeeded, or activeBook would stay
    -- pointing at a shut book forever
    shared.activeBook = nil
    if not shared.uiFocused then return end   -- no focus was taken: nothing to hand back
    shared.uiFocused = false
    u.call(pc, m.inputGameFn or "SetInputModeGame", false, false, false)
    if wbl then
      pcall(function() wbl:SetInputMode_GameOnly(pc, false) end)
    end
    u.set(pc, "bShowMouseCursor", false)
  end
end

-- Belt & braces for the repoint: SetInputModeGame is the chokepoint EVERY UI close funnels
-- through (same one the wand's hand-rebuild rides). If it fires while a book is repointed and
-- the cover is already shut (a close path that dodged the widget hooks), restore the real guide.
-- Armed ONCE for both books -- it asks the owner, so a second registration would buy nothing.
local function armGameModeHook(pc, b)
  if shared.gameModeHooked then return end
  local m = b.map()
  local cls
  pcall(function() cls = pc:GetClass():GetFullName() end)
  if not cls then return end
  local path = (cls:gsub("^%S+%s+", "")) .. ":" .. (m.inputGameFn or "SetInputModeGame")
  shared.gameModeHooked = pcall(RegisterHook, path, ctx.log.guard("books.gamemode", function()
    onGameThread(function()
      local owner = shared.repointOwner
      if not owner then return end
      local vis = false
      pcall(function() vis = owner.widget:IsVisible() end)
      -- setUiFocus(false) re-calls SetInputModeGame once; re-entry no-ops on cleared flags
      if not vis then
        setUiFocus(false, owner)
        restoreGuide()
      end
    end)
  end))
end

-- ---------------------------------------------------------------- one book
local function newBook(spec)
  local self = {
    mapKey  = spec.mapKey,
    label   = spec.label,
    cmd     = spec.cmd,
    widget  = nil,            -- the created W_<book> instance (one per local player, reused)
    hooked  = false,
    warned  = false,          -- "pak missing" is logged once, not per interact attempt
    -- Why the last ensureWidget attempt failed (surfaced in the interact-time warn -- the
    -- boot-window pin race taught us that silent returns cost a whole live session to diagnose).
    whyNot  = "not attempted",
    closeHooked = false,
    -- no research card => nothing to migrate
    migrated = not spec.research,
  }

  function self.map() return ctx.map[self.mapKey] or {} end

  -- Find the widget class -- FIND-ONLY, never a native load. Two dead ends, both proven live
  -- 2026-07-21, are documented here so nobody retries them:
  --   * UE4SS LoadAsset cannot load our pak's packages (no AssetRegistry entry; classByName's
  --     LoadAsset fallback silently no-ops -- harmless, just useless here).
  --   * KismetSystemLibrary.LoadClassAsset_Blocking exists on this build, but UE4SS CANNOT
  --     marshal its TSoftClassPtr param: the call goes native with a garbage soft path and FATALS
  --     the game (the 23:45 menu-idle launch crash; pcall cannot catch a native fault). NEVER
  --     call the *_Blocking soft-path loaders from Lua.
  -- So the chain must be ROOTED, not re-loaded. The keeper recipe row alone proved NOT to hold it
  -- (live 2026-07-22: "widget class not resident" one minute into a session), so the pak also
  -- plants a script OBJECT REFERENCE to each W_<book>_C in its placeable's interact event
  -- bytecode (see build_wand_pak.py): while any book stands placed, its class roots that reader
  -- chain. This feature still parks each widget in the viewport as belt-and-braces.
  local function loadWidgetClass(m)
    local cls = ctx.uehelp.classByName(m.widgetClass, m.widgetPath)
    if cls then return cls end
    -- FALLBACK (live 2026-07-23): classByName's FindObject(kind, name) query returned nil for all
    -- three kinds while StaticFindObject(widgetPath) DID hand back the class object -- the pak was
    -- proven complete offline that same night (ClassExport + both rooting imports intact), so the
    -- miss is in the name query, not the asset. Look it up by full path, but never hand Create()
    -- an object that is not a generated class: that call goes native.
    local o
    pcall(function() o = StaticFindObject and StaticFindObject(m.widgetPath) end)
    if ctx.uehelp.isValid(o) then
      local kind = ctx.uehelp.className(o)
      if kind == "WidgetBlueprintGeneratedClass" or kind == "BlueprintGeneratedClass" then
        return o
      end
      self.whyNot = "object at the widget path is a " .. tostring(kind) .. ", not a widget class"
      return nil
    end
    self.whyNot = "widget class not resident (UI chain GC'd; is the pak current?)"
    return nil
  end

  -- Create the widget once and PARK IT IN THE VIEWPORT (hidden by design, exactly like the
  -- controller pre-creates W_SurvivalGuide in StartupUI): a created, viewport-held widget is a
  -- rooted reference that keeps the class chain alive for the rest of the session. The pin CANNOT
  -- rely on the load window alone (proven live 2026-07-21): the chain loads at BOOT (DB_Buildables
  -- roots the placeable class, whose import edge drags the UI in ~2s after process start), when no
  -- PlayerController exists to own a widget, and the post-load GC evicts it seconds later --
  -- loadWidgetClass's on-demand re-load is what makes ensureWidget work at interact time.
  function self.ensureWidget()
    local u, m = ctx.uehelp, self.map()
    if u.isValid(self.widget) then return true end
    local pc = u.playerController()
    if not pc then self.whyNot = "no player controller (boot/menu)"; return false end
    local cls = loadWidgetClass(m)
    if not cls then return false end
    local wbl = StaticFindObject and StaticFindObject(m.wblPath)
    if not wbl then self.whyNot = "no WidgetBlueprintLibrary"; return false end
    local w
    pcall(function() w = wbl:Create(pc, cls, pc) end)
    if not u.isValid(w) then
      self.whyNot = "widget create failed"
      ctx.log.warn(self.label .. ": widget create failed")
      return false
    end
    pcall(function() w:AddToViewport(50) end)
    self.widget = w
    self.warned = false
    ctx.log.info(self.label .. ": the book is bound (widget pinned in viewport)")
    return true
  end

  -- Race the post-load GC: try soon, retry a few times (the player controller can lag the class
  -- load by a beat during world load). Delay chain, never a poll.
  function self.pinSoon(tries)
    tries = tries or 4
    local function attempt(n)
      onGameThread(function()
        if self.ensureWidget() then return end
        if n <= 1 then
          -- surface the loss (a silent pin failure cost a whole live session to diagnose); the
          -- placeable's cooked script-ref root makes this rare -- see build_wand_pak.py
          ctx.log.info(self.label .. ": widget pin not settled yet -- " .. tostring(self.whyNot) ..
            " (interact re-tries)")
          return
        end
        if not pcall(ExecuteWithDelay, 700, function() attempt(n - 1) end) then attempt(n - 1) end
      end)
    end
    if not pcall(ExecuteWithDelay, 250, function() attempt(tries) end) then attempt(tries) end
  end

  -- The widget's Close/Hide BP events fire for the X button AND for the game's own close paths
  -- (ESC -> CloseOpenInteractableUIs, cinematics -> HideAllUI, both reach us via the repoint):
  -- hook them to hand game input back and un-repoint. (Real work deferred out of the BP call
  -- chain, as everywhere.) Per book: the hook paths are this widget's own.
  local function armCloseHooks()
    if self.closeHooked then return end
    local m = self.map()
    for _, fn in ipairs(m.closeFns or { "Close", "Hide" }) do
      local ok = pcall(RegisterHook, (m.widgetPath or "") .. ":" .. fn,
        ctx.log.guard(self.label .. ".close", function()
          onGameThread(function()
            -- Never steal focus or the slot from the OTHER book. These are two SEPARATE
            -- questions, and conflating them is a real bug: opening book B shuts book A, and
            -- A's close handler lands one beat later -- by which time B may already own the
            -- repoint while A still owns the screen. An `or` here would un-repoint B and leave
            -- the player reading with movement unlocked.
            if shared.activeBook == self then setUiFocus(false, self) end
            if shared.repointOwner == self then restoreGuide() end
          end)
        end))
      self.closeHooked = self.closeHooked or ok
    end
    if self.closeHooked then
      ctx.log.info(self.label .. ": close hooks armed (input restores on shut cover)")
    end
  end

  -- Create (once) + Open this book for the local player, taking UI input focus.
  --
  -- The focus grab is DEFERRED out of the interact frame (live 2026-07-22): our interact hook is a
  -- PRE-hook, so the cooked placeable's own event body -- whose tail is the retargeted
  -- ForceCloseInteractableUIs, i.e. the controller's generic interactable-UI reset ending in
  -- SetInputModeGame -- runs AFTER an in-frame SetInputModeUI and silently reverts it. That is why
  -- the book showed but WASD/hotbar keys kept working. One beat later wins that race; a second
  -- beat re-asserts in case a same-frame close chokepoint lands late.
  function self.open()
    local u, m = ctx.uehelp, self.map()
    if not self.ensureWidget() then
      if not self.warned then
        self.warned = true
        ctx.log.warn(self.label .. ": cannot open the book -- " .. self.whyNot)
      end
      return
    end
    -- one book on screen at a time: shut the other cover so its own close hook un-repoints
    local other = shared.activeBook
    if other and other ~= self and u.isValid(other.widget) then
      pcall(function() other.widget:Close() end)
      if shared.repointOwner and shared.repointOwner ~= self then restoreGuide() end
    end
    armCloseHooks()
    local pc = u.playerController()
    if pc then
      armGameModeHook(pc, self)
      repointGuide(pc, self)   -- BEFORE Open: the pawn's input gates read IsVisible through it
    end
    u.call(self.widget, m.openFn or "Open")
    local function grab()
      onGameThread(function()
        -- only while the cover is actually open: a fast X-click between beats must not leave the
        -- player cursor-locked with no UI on screen
        local vis = false
        pcall(function() vis = self.widget:IsVisible() end)
        if ctx.uehelp.isValid(self.widget) and vis then setUiFocus(true, self) end
      end)
    end
    if not pcall(ExecuteWithDelay, 200, grab) then grab() end
    pcall(ExecuteWithDelay, 700, function()
      if shared.uiFocused and shared.activeBook == self then grab() end
    end)
  end

  -- "The Dark Arts" research card, old-save migration. The card is tier-2 gated data-side
  -- (StartingResearch=False; LvL_2's UnlockingResearchIDs reveals it), but a save that researched
  -- LvL_2 BEFORE this pak existed never re-fires that unlock list, so the card would stay
  -- invisible forever. Availability is just membership in the player's saved Researches array
  -- (S_SavedResearch {id, Researched=false} -- RE'd from HasPlayerResearch?/UnlockResearch), and
  -- the controller's own Playerdata_SaveResearchForSelf plants exactly one entry; UE4SS marshals
  -- the struct param from a Lua table keyed by the BP-struct's suffixed field names (proven live
  -- 2026-07-22 on the user's world: can=false -> can=true). Local player only; each machine runs
  -- its own migration, and the save flow is the game's own host-authoritative path.
  -- Books with no research card map no research* keys and skip all of this.
  local function migrateResearch()
    if self.migrated then return true end
    local u, m = ctx.uehelp, self.map()
    if not (m.researchId and m.researchTierId and m.researchHasFn and m.researchSaveFn
        and m.researchFieldId and m.researchFieldDone) then return true end
    local pc = u.playerController()
    if not pc then return false end
    -- only touch the save when our pak is demonstrably active (a planted id with no table row
    -- would hand the research UI a rowless card)
    if not u.classByName(m.placeableClass, m.placeablePath) then return false end
    local function has(id)
      local t = {}
      if not pcall(function() pc[m.researchHasFn](pc, id, t, t) end) then return nil end
      return t.CanResearch == true, t.IsResearched == true
    end
    local _, tierDone = has(m.researchTierId)
    if tierDone == nil then return false end          -- playerdata not loaded yet; retry
    local cardCan, cardDone = has(m.researchId)
    if cardCan == nil then return false end
    -- the card is settled if it is already OFFERED (can) or already COMPLETED (done): re-planting
    -- a completed card as {Researched=false} would resurrect it in the station every session
    if not tierDone or cardCan or cardDone then self.migrated = true; return true end
    local ok = pcall(function()
      pc[m.researchSaveFn](pc, {
        [m.researchFieldId]   = m.researchId,
        [m.researchFieldDone] = false,
      })
    end)
    if ok then
      self.migrated = true
      ctx.log.info(self.label .. ": The Dark Arts surfaces in this elder world (card migrated)")
    end
    return ok
  end

  -- Playerdata lags world entry (it loads during the join flow), so try on a spaced delay chain --
  -- never a poll -- and stop as soon as one attempt settles the question.
  function self.migrateSoon()
    if self.migrated then return end
    local delays = { 4000, 12000, 30000 }
    local function attempt(n)
      onGameThread(function()
        if migrateResearch() then return end
        local nxt = delays[n + 1]
        if nxt and pcall(ExecuteWithDelay, nxt, function() attempt(n + 1) end) then return end
      end)
    end
    if not pcall(ExecuteWithDelay, delays[1], function() attempt(1) end) then attempt(1) end
  end

  -- Arm the interact hook on this book's cooked placeable bound event. The class is loaded on
  -- demand (LoadAsset via classByName), so this works before any copy has ever been placed.
  function self.hookInteract()
    if self.hooked then return end
    local u, m = ctx.uehelp, self.map()
    local cls = u.classByName(m.placeableClass, m.placeablePath)
    if not cls then return end
    local paths = {}
    pcall(function()
      cls:ForEachFunction(function(fn)
        local n = ""; pcall(function() n = fn:GetFName():ToString() end)
        if n:find(m.interactFnHint or "OnInteractedWith", 1, true) then
          local full; pcall(function() full = fn:GetFullName() end)
          if full then paths[#paths + 1] = (full:gsub("^%S+%s+", "")) end
        end
      end)
    end)
    for _, path in ipairs(paths) do
      local ok = pcall(RegisterHook, path, ctx.log.guard(self.label .. ".interact",
        function(Context, comp, hit, controller, tool)
          local pc; pcall(function() pc = controller:get() end)
          -- real work OUT of the hook call chain (never mutate from inside the BP call)
          onGameThread(function()
            local localPc = ctx.uehelp.playerController()
            -- open only on the machine whose local player did the interacting (on the host the
            -- event also fires for remote players' controllers -- their UI is not ours to open).
            -- isValid, never `== nil`: a null-UObject wrapper is not Lua nil, and touching one
            -- native-crashes
            if not ctx.uehelp.isValid(pc) or ctx.uehelp.sameObject(pc, localPc) then self.open() end
          end)
        end))
      if ok then self.hooked = true end
    end
    if self.hooked then
      ctx.log.info(self.label .. ": the placed book listens (" .. #paths .. " interact hook)")
      -- arm time == the placeable class just loaded == the widget chain is briefly resident: pin it
      self.pinSoon()
    end
  end

  function self.onWorldEntry()
    if not pcall(ExecuteWithDelay, 200, function() onGameThread(self.hookInteract) end) then
      onGameThread(function() self.hookInteract() end)
    end
    self.pinSoon()
    self.migrateSoon()
  end

  return self
end

-- ---------------------------------------------------------------- feature
local SPECS = {
  { mapKey = "codex",    cmd = "sps_codex",    label = "codex",    research = true  },
  { mapKey = "handbook", cmd = "sps_handbook", label = "handbook", research = false },
}

function F.init(c)
  ctx = c
  books = {}
  for _, spec in ipairs(SPECS) do
    -- each book gates independently: a pak that ships one and not the other still works
    local keys = { spec.mapKey .. ".widgetClass", spec.mapKey .. ".widgetPath",
                   spec.mapKey .. ".placeableClass", spec.mapKey .. ".placeablePath" }
    if ctx.gate.require(ctx.log, ctx.map, spec.mapKey, keys) then
      books[#books + 1] = newBook(spec)
    end
  end
  if #books == 0 then return false end

  onGameThread(function()
    for _, b in ipairs(books) do b.hookInteract() end
  end)
  for _, b in ipairs(books) do
    if not b.hooked then
      ctx.log.info(b.label .. ": placeable class not loadable yet (pak missing or not mounted) -- "
        .. b.cmd .. " retries and opens the book directly")
    end
  end

  -- World entry re-arms the interact hooks, re-pins the widget chains, and triggers the old-save
  -- card migration -- all off ONE trigger: a pawn constructing, the same rare, proven-safe notify
  -- the wand rides. Registered ONCE for both books. NEVER a bare-Actor or controller construction
  -- notify: a 2026-07-22 world-entry fatal (60s game-thread hang) bisected to exactly such
  -- notifies firing in the world-load actor storm. Mid-session placements need no notify of their
  -- own -- hookInteract armed at init covers the class, and open() re-resolves the chain per read.
  ctx.uehelp.onNewInstance("/Script/Engine.Character", nil, function()
    -- construction callback: defer everything out of the spawn stack (pinSoon already delays)
    -- world change: the old controller (and any repoint into it) died with the old world
    shared.repointOwner, shared.guideOriginal = nil, nil
    shared.activeBook, shared.uiFocused = nil, false
    for _, b in ipairs(books) do b.onWorldEntry() end
  end)

  for _, b in ipairs(books) do
    b.migrateSoon()  -- already in-world (hot reload / late init)
    pcall(function()
      RegisterConsoleCommandHandler(b.cmd, function()
        onGameThread(function()
          b.hookInteract()   -- retry arming in case the pak mounted after init
          b.open()
        end)
        return true
      end)
    end)
  end
  return true
end

return F
